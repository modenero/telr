...
/*******************************************************************************
 *
 * @notice TelrCache IS THE "OFFICIAL" META-CURRENCY OF THE GROWING COMMUNITY
 *         OF TELR-SPONSORED PRODUCTS AND SERVICES.
 *
 * @dev In conjunction with the TelrCache Daemon, this contract manages the
...
contract TelrCache is Owned {
    ...
    /* Initialize Modnero Db contract. */
    ModneroDbInterface private _modeneroDb;
     ...
     *       ModneroDb keys; in order to prevent ANY accidental or
     *       malicious SQL-injection vulnerabilities / attacks.
     */
    string private _namespace = 'telr-cache';
    ...
    //       from legacy TelrCache contract instance(s).
     ...
     */
    constructor() public {
        /* Initialize ModneroDb (eternal) storage database contract. */
        // NOTE We hard-code the address here, since it should never change.
        // _modeneroDb = ModneroDbInterface(0xE865Fe1A1A3b342bF0E2fcB11fF4E3BCe58263af);
        _modeneroDb = ModneroDbInterface(0x3e246C5038287DEeC6082B95b5741c147A3f49b3); // AMBER
        ...
        if (_predecessor != 0x0) {
            ...
            uint lastRevision = TelrCache(_predecessor).getRevision();
    ...
    /**
     * @dev Only allow access to an authorized Modnero administrator.
     */
     ...
     * in the TelrCache, FOR COMPLIANCE PURPOSES ONLY.
     *
     * NOTE: This function is reserved for exclusive use by
     *       Modnero Administration ONLY.
     *
     ...
     * Withdraw
     *
     * Allows the withdrawl of tokens held in the TelrCache
     ...
     *       possible rogue instance(s) of TelrCache.
     */
     ...
     * from legacy instance(s) to the LATEST instance of TelrCache.
     */
    function _migrate(
        address _owner,
        address[] _tokens
        ...
        /* Retrieve value from Modnero Db. */
        address latestCache = _modeneroDb.getAddress(hash);
    ...
    function balanceOf(
        ...
        if (legacyInstance != 0x0) {
            ...
            for (uint i = 0; i < _depth; i++) {
                /* Retrieve balance. */
                uint legacyBalance = TelrCache(legacyInstance)
                ...
                legacyInstance = TelrCache(legacyInstance).getPredecessor();
     ...
     * using the aname record from ModneroDb.
     */
