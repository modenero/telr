 ...
 *
 * Telr Exchange - A 100% non-custodial, cross-chain exchange.
 *
 * Version 20.11.6
 *
 * https://modenero.com
...
contract TelrExchange is Owned {
    ...
    //       from legacy Telr Exchange contract instance(s).
     ...
     * in the Telr Exchange, FOR COMPLIANCE PURPOSES ONLY.
     *
     ...
     * Withdraw
     *
     * Allows the withdrawl of tokens held in the Telr Exchange
    ...
    function balanceOf(
        ...
        if (legacyInstance != address(0x0)) {
            ...
            for (uint i = 0; i < _depth; i++) {
                /* Retrieve balance. */
                uint legacyBalance = TelrExchange(legacyInstance)
                ...
                legacyInstance = TelrExchange(legacyInstance).getPredecessor();
